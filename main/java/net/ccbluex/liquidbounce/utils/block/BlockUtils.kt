...
object BlockUtils : MinecraftInstance() {
        ...
        for (var11 in var4 until var5) {
            for (var12 in var6 until var7) {
                for (var13 in var8 until var9) {
                    ...
                    val var14 =
                        Minecraft.getMinecraft().theWorld.getBlockState(pos).block
    ...
    @JvmStatic
    fun isReallyOnGround(): Boolean {
        val entity: EntityPlayerSP = Minecraft.getMinecraft().thePlayer
        val y: Double = entity.entityBoundingBox.offset(0.0, -0.01, 0.0).minY
        val block =
            Minecraft.getMinecraft().theWorld.getBlockState(BlockPos(entity.posX, y, entity.posZ)).block
        return if (block != null && block !is BlockAir && block !is BlockLiquid) {
            entity.onGround
        } else false
    }
