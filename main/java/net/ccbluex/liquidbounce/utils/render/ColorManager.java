...
public class ColorManager {
    ...
    public static Color astolfoRainbow(int var2, float bright, float st, double speed) {
    ...
    public static int getNewRainbow(int delay, long timeOffset,float saturation) {
        double rainbowState = Math.ceil((System.currentTimeMillis() + timeOffset) / 8 + delay / 20.0D);
        rainbowState %= 360.0;
        return Color.getHSBColor((float) (rainbowState / 360f), saturation, 1F).getRGB();
    }
