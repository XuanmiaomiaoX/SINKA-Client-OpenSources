...
public class FileManager extends MinecraftInstance {
    ...
    public final File dir = new File(mc.mcDataDir, "SINKAClient-1.8");
    ...
    public void saveConfignoMSG(final FileConfig config) {
        saveConfigNoMSG(config, false);
    }
    ...
    private void saveConfigNoMSG(final FileConfig config, final boolean ignoreStarting) {
        if (!ignoreStarting && LiquidBounce.INSTANCE.isStarting())
            return;
