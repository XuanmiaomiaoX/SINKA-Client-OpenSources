...
class Arraylist(x: Double = 0.0, y: Double = 1.0, scale: Float = 1.00F, side: Side = Side(Horizontal.RIGHT, Vertical.UP)) : Element(x, y, scale, side) {
    private val nameBreakValue = BoolValue("NameBreak", false)
    private val outLineRectWidth = IntegerValue("OutLineRectWidth", 3, 3, 6)
    private val animationSpeedValue = FloatValue("AnimationSpeed", 0.5F, 0.01F, 0.5F)
    private val rainbow3Offset = IntegerValue("RainbowOffset", 16, 1, 30)
    private val rainbowSpeed = IntegerValue("RainbowSpeed",4,1,10)
    private val rainbowSaturation = FloatValue("RainbowSaturation", 0.48F,0.01F,1F)
    ...
    private val colorModeValue = ListValue("Text-Color", arrayOf("Custom", "Random", "Rainbow","Rainbow3","CustomRainbow", "Flux","Astolfo", "Gident"), "Gident")
    private val colorRedValue = IntegerValue("Text-R", 255, 0, 255)
    private val colorGreenValue = IntegerValue("Text-G", 110, 0, 255)
    private val colorBlueValue = IntegerValue("Text-B", 160, 0, 255)

    private val gidentspeed = IntegerValue("GidentSpeed", 180, 1, 1000)
    private val rectColorModeValue = ListValue("Rect-Color", arrayOf("Custom", "Random", "Rainbow" , "Rainbow3", "CustomRainbow", "Flux","Astolfo", "Gident"), "Gident")
    private val rectColorRedValue = IntegerValue("Rect-R", 255, 0, 255)
    private val rectColorGreenValue = IntegerValue("Rect-G", 80, 0, 255)
    private val rectColorBlueValue = IntegerValue("Rect-B", 121, 0, 255)
    ...
    private val backgroundColorAlphaValue = IntegerValue("Background-Alpha", 135, 0, 255)
    private val rectValue = ListValue("Rect", arrayOf("None", "Left", "Right", "OutLine"), "Right")
    private val upperCaseValue = BoolValue("UpperCase", false)
    private val spaceValue = FloatValue("Space", 0F, 0F, 5F)
    private val textHeightValue = FloatValue("TextHeight", 9.97F, 1F, 20F)
    private val textYValue = FloatValue("TextY", 1.5F, 0F, 20F)
    private val tagsArrayColor = BoolValue("TagsArrayColor", false)
    private val fontValue = FontValue("Font", Fonts.comfortaa35)
    private var x2 = 0
    private var y2 = 0F
    ...
    override fun drawElement(): Border? {
                ...
                modules.forEachIndexed { index, module ->
                    ...
                    color3 = RenderUtils2.getGradientOffset(Color(ClientColor.colorRedValue.get(), ClientColor.colorGreenValue.get(), ClientColor.colorBlueValue.get(),1), Color(ClientColor.colorRedValue2.get(), ClientColor.colorGreenValue2.get(), ClientColor.colorBlueValue2.get(),1), (Math.abs(
                        System.currentTimeMillis() / gidentspeed.get()
                            .toDouble() + (module.translate.y / fontRenderer.FONT_HEIGHT)
                    ) / 10)).rgb
                    ...
                    if (!rectMode.equals("none", true)) {
                        RainbowShader.begin(rectMode == "Rainbow", if (rainbowX.get() == 0.0F) 0.0F else 1.0F / rainbowX.get(), if (rainbowY.get() == 0.0F) 0.0F else 1.0F / rainbowY.get(), System.currentTimeMillis() % 10000 / 10000F).use {
                            ...
                            when(rectMode.toLowerCase()) {
                                "left" -> RenderUtils.drawRect(xPos - 3, translate.y, xPos-3, translate.y + textHeight,
                                    rectColor)
                                "right" -> RenderUtils.drawRect(-3F, module.translate.y - 1F, -2F,
        ...
        if (mc.currentScreen is GuiHudDesigner) {
            ...
            return Border(0F, 0F, x2 - 5F, y2 - if(side.vertical == Vertical.DOWN) 1F else 0F)
        }
