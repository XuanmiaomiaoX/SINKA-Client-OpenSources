...
import net.ccbluex.liquidbounce.utils.render.BlurUtilsFdp
...
abstract class Element(var x: Double = 2.0, var y: Double = 2.0, var scale: Float = 1F,
    ...
    protected val blurValue = FloatValue("Blur", 0f, 0f, 100f)
    ...
    open fun drawBoarderBlur(blurRadius: Float = blurValue.get()){
        ...
        BlurUtilsFdp.doBlur(posX*scale, posY*scale, width*scale, height*scale, blurRadius)
    }
    ...
    protected fun blur(x: Float, y: Float, x2: Float, y2: Float){
        ...
        BlurUtilsFdp.doBlur((renderX+(x.coerceAtMost(x2))).toFloat()*scale, (renderY+(y.coerceAtMost(y2))).toFloat()*scale, abs(x2-x)*scale, abs(y2-y)*scale, blurValue.get())
    }
...
annotation class ElementInfo(val name: String, val single: Boolean = false, val force: Boolean = false, val blur: Boolean = false)
...
data class Border(val x: Float, val y: Float, val x2: Float, val y2: Float) {
    ...
    fun draw() = RenderUtils.drawBorderedRectBlur(x, y, x2, y2, 3F, Int.MIN_VALUE, 0)
}