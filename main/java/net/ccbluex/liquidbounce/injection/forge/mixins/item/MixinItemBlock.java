...
import net.ccbluex.liquidbounce.features.module.modules.misc.FixBlockSound;
    ...
    public boolean onItemUse(ItemStack p_onItemUse_1_, EntityPlayer p_onItemUse_2_, World p_onItemUse_3_, BlockPos p_onItemUse_4_, EnumFacing p_onItemUse_5_, float p_onItemUse_6_, float p_onItemUse_7_, float p_onItemUse_8_) {
        ...
        if (p_onItemUse_1_.stackSize == 0) {
            ...
            if (itemBlock.placeBlockAt(p_onItemUse_1_, p_onItemUse_2_, p_onItemUse_3_, p_onItemUse_4_, p_onItemUse_5_, p_onItemUse_6_, p_onItemUse_7_, p_onItemUse_8_, iblockstate1)) {
                if(LiquidBounce.moduleManager.getModule(FixBlockSound.class).getState()) {
                    mc.theWorld.playSoundAtPos(p_onItemUse_4_, itemBlock.block.stepSound.getStepSound(), (itemBlock.block.stepSound.getVolume() + 1.0F) / 2.0F, itemBlock.block.stepSound.getFrequency() * 0.8F, false);
                }else{
                    p_onItemUse_3_.playSoundEffect((double)((float)p_onItemUse_4_.getX() + 0.5F), (double)((float)p_onItemUse_4_.getY() + 0.5F), (double)((float)p_onItemUse_4_.getZ() + 0.5F), itemBlock.block.stepSound.getPlaceSound(), (itemBlock.block.stepSound.getVolume() + 1.0F) / 2.0F, itemBlock.block.stepSound.getFrequency() * 0.8F);
                }
