...
import net.ccbluex.liquidbounce.utils.Skid.EntityUtils2
...
class CombatManager : Listenable,MinecraftInstance() {
    ...
    fun onUpdate(event: UpdateEvent){
        ...
        for (entity in mc.theWorld.loadedEntityList) {
            if (entity is EntityLivingBase
                && entity.getDistanceToEntity(mc.thePlayer) < 7 && EntityUtils2.isSelected(entity, true)) {
                inCombat = true
                break
            }
        }
    ...
    fun onAttack(event: AttackEvent){
        ...
        if(target is EntityLivingBase && EntityUtils2.isSelected(target,true)){
            this.target=target
    ...
    fun getNearByEntity(radius: Float):EntityLivingBase?{
        return try {
            mc.theWorld.loadedEntityList
                .filter { mc.thePlayer.getDistanceToEntity(it)<radius&&EntityUtils2.isSelected(it,true) }
