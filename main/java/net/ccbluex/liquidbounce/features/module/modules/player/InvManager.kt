...
@ModuleInfo(name = "InvManager", description = "Automatically throws away useless items.", category = ModuleCategory.PLAYER)
class InvManager : Module() {
    ...
    private val itemDelayValue = IntegerValue("ItemDelay-MS", 0, 0, 5000)
    ...
    private val nbtItemNotGarbage = BoolValue("NBTItemNotGarbage", true)
    private val nbtArmorPriority = FloatValue("NBTArmorPriority", 0f, 0f, 5f)
    private val nbtWeaponPriority = FloatValue("NBTWeaponPriority", 0f, 0f, 5f)
    ...
    fun performManager() {
        ...
        if (hotbarValue.get() && mc.currentScreen !is GuiInventory) {
            for (index in 0..8) {
    ...
    fun isUseful(itemStack: ItemStack, slot: Int): Boolean {
        return try {
            ...
            if (item is ItemSword || item is ItemTool) {
                if (slot >= 36 && findBetterItem(slot - 36, mc.thePlayer.inventory.getStackInSlot(slot - 36)) == slot - 36&&mc.currentScreen is GuiInventory) {
                    return true
                }
