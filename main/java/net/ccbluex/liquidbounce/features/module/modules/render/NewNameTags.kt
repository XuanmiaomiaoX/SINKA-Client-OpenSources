...
@ModuleInfo(name = "NewNameTags", description = "Name Render" ,category = ModuleCategory.RENDER)
class NewNameTags : Module() {
    private val modeValue = ListValue("Mode", arrayOf("Sinka","Simple", "Liquid", "Jello"), "Sinka")
        ...
        for (entity in mc.theWorld.loadedEntityList) {
            if (EntityUtils.isSelected(entity, false)) {
                renderNameTag(entity as EntityLivingBase,
                    if (hackerValue.get() && isHacker(entity)) { "§c" } else { "" } + if (!modeValue.equals("Liquid") && AntiBot.isBot(entity)) { "§e" } else { "" } +
    ...
    private fun renderNameTag(entity: EntityLivingBase, tag: String) {
        ...
        when (modeValue.get().toLowerCase()) {
            ...
            "sinka" -> {
                ...
                // render bg
                glScalef(-scale * 2, -scale * 2, scale * 2)
                drawRoundRect(-width - 4F, -fontRenderer.FONT_HEIGHT * 3F, width + 4F, -3F,5.0f, Color(ClientColor.red, ClientColor.green, ClientColor.blue,ClientColor.air1).rgb)

                // string
                fontRenderer.drawString(tag, -width, -fontRenderer.FONT_HEIGHT * 2 - 4,Color(ClientColor.red1, ClientColor.green1, ClientColor.blue1).getRGB())
                glScalef(0.5F, 0.5F, 0.5F)
                fontRenderer.drawString("Health: " + entity.health.toInt(), -width * 2, -fontRenderer.FONT_HEIGHT * 2,Color(ClientColor.red1, ClientColor.green1, ClientColor.blue1).getRGB())
            }
        }
        // Reset caps
        resetCaps()
