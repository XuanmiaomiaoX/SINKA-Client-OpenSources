...
@ModuleInfo(name = "NameTag", description = "=/", category = ModuleCategory.RENDER)
            ...
            if (entity instanceof EntityLivingBase) {
                ...
                if (isValid(ent) && RenderUtils.isInViewFrustrum(ent) && entity.getUniqueID() != mc.thePlayer.getUniqueID()) {
                    ...
                    if (position != null) {
                        ...
                        if (background.get() > 0) {
                            RenderUtils.MoondrawRect((x + (x2 - x) / 2) - (!font.get().equals(Fonts.minecraftFont) ? font.get().getStringWidth(nameText) : mc.fontRendererObj.getStringWidth(nameText)) / 2f - 2.5,
