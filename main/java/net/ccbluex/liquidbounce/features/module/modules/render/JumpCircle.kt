...
@ModuleInfo(name = "JumpCircle",description = "Jump Darw Circle", category = ModuleCategory.RENDER)
class JumpCircle : Module() {

    val disappearTime = IntegerValue("Time", 1000, 1000,3000)
    val radius = FloatValue("Radius", 2f, 1f,5f)

    val rainbow = BoolValue("Rainbow", true)
    ...
    class Circle(val time: Long, val x: Double, val y: Double, val z: Double){
        ...
        fun draw() {
            ...
            for (i in 0..360) {
                val color = if (jumpModule.rainbow.get()) Color.getHSBColor(i / 360f, 0.5f, 1f)
