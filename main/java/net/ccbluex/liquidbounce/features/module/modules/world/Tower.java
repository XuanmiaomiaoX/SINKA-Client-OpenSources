...
public class Tower extends Module {
    ...
    // Rotation lock
    private net.ccbluex.liquidbounce.utils.Rotation lockRotation;
            ...
            if (autoBlockValue.get() ? InventoryUtils.findAutoBlockBlock() != -1 : mc.thePlayer.getHeldItem() != null
                ...
                if (mc.theWorld.getBlockState(blockPos).getBlock() instanceof BlockAir) {
                    if (search(blockPos) && rotationsValue.get()) {
                        final net.ccbluex.liquidbounce.utils.Skid.VecRotation vecRotation = RotationUtils.faceBlock(blockPos);
            ...
            for (double xSearch = 0.1D; xSearch < 0.9D; xSearch += 0.1D) {
                for (double ySearch = 0.1D; ySearch < 0.9D; ySearch += 0.1D) {
                    for (double zSearch = 0.1D; zSearch < 0.9D; zSearch += 0.1D) {
                        ...
                        final net.ccbluex.liquidbounce.utils.Rotation rotation = new net.ccbluex.liquidbounce.utils.Rotation(
