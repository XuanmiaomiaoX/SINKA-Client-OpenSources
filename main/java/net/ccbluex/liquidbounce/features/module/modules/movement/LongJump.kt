...
@ModuleInfo(name = "LongJump", description = "LongJump" , category = ModuleCategory.MOVEMENT)
class LongJump : Module() {
    private val modeValue = ListValue("Mode", arrayOf("NCP", "NCPDamage", "JartexWater","DCJ","AACv1", "AACv2", "AACv3", "Mineplex", "Mineplex2", "Mineplex3", "RedeSkyTest", "RedeSky", "RedeSky2", "RedeSky3", "BlocksMC", "OldBlocksMC2", "HYT4v4"), "RedeSky2")
    ...
    // ncp damage
    private val ncpdInstantValue = BoolValue("NCPDamageInstant", false)
    // jartex network
    private val jartexYValue = FloatValue("JartexMotionY", 0.42F, 0.0F, 2F)
    private val jartexHValue = FloatValue("JartexHorizon", 1.0F, 0.8F, 4F)
    // settings
        ...
        if (jumped) {
            ...
            run {
                when (mode.toLowerCase()) {
                    ...
                    "blocksmc" -> {
                        MovementUtils.strafe(MovementUtils.getSpeed() * if (canBoost) 6.0f else 1f)
