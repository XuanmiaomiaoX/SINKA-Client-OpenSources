...
@ModuleInfo(name = "KillAura", category = ModuleCategory.COMBAT, keyBind = Keyboard.KEY_R, description = "Cause damage to this area")
class KillAura : Module() {
    ...
    private val particles = BoolValue("Aura Particles", true)
    ...
    private val markValue = ListValue("Mark", arrayOf("Liquid","Sinka" ,"Block", "OtherBlock", "Jello", "Sims", "None"), "Sinka").displayable { visualDisplay.get() }
    ...
    private val circleRainValue = BoolValue("CircleRainbow", true).displayable { visualDisplay.get() }
    ...
    fun onMotion(event: MotionEvent) {
        ...
        updateHitable()
        val target1 = this.target ?: discoveredTargets.getOrNull(0) ?: return

        if (autoBlockValue.equals("Range") && autoBlockPacketValue.equals("HoldKey") && canBlock) {
            if (inRangeDiscoveredTargets.isEmpty()) {
                mc.gameSettings.keyBindUseItem.pressed = false
            } else if (mc.thePlayer.getDistanceToEntityBox(target1) < maxRange) {
        ...
        // AutoBlock
        if (!autoBlockValue.get().equals("off",true) && discoveredTargets.isNotEmpty() && (!autoBlockPacketValue.get().equals("Attack",true)||discoveredTargets.filter { mc.thePlayer.getDistanceToEntityBox(it)>maxRange }.isNotEmpty()) && canBlock) {
            val target111=discoveredTargets[0]
            if(mc.thePlayer.getDistanceToEntityBox(target1) < autoBlockRangeValue.get())
                startBlocking(target1, interactAutoBlockValue.get() && (mc.thePlayer.getDistanceToEntityBox(target111) < maxRange))
        }
    ...
    fun onUpdate(ignoredEvent: UpdateEvent) {
        if (particles.get()){
            LiquidBounce.moduleManager[Particles::class.java]!!.state=true
        }
        if(target!=null && target!!.hurtTime>=9){
            (LiquidBounce.moduleManager[Particles::class.java] as Particles).omg=true
        }
        ...
        var strafeFix = LiquidBounce.moduleManager.getModule(StrafeFix::class.java) as StrafeFix
        strafeFix.applyForceStrafe(rotationStrafeValue.equals("Silent"), !rotationStrafeValue.equals("Off") && !rotationModeValue.equals("None"))
    ...
    private fun preAttack() {
        if (mc.thePlayer.isBlocking || blockingStatus) {
            when (autoBlockPacketValue.get().toLowerCase()) {
                "vanilla" -> null
    ...
    private fun updateHitable() {
        ...
        hitable = RotationUtils.isFaced(target, maxRange.toDouble()) && (entityDist < throughWallsRangeValue.get() || wallTrace?.typeOfHit != MovingObjectPosition.MovingObjectType.BLOCK) && (target as EntityLivingBase).hurtTime <= hurtTimeValue.get()
    }
    ...
    private fun startBlocking(interactEntity: Entity, interact: Boolean) {
        ...
        if (interact) {
            ...
            val boundingBox = interactEntity.entityBoundingBox
            ...
            val movingObject = boundingBox.calculateIntercept(positionEye, lookAt) as MovingObjectPosition ?: return
    ...
    fun onRender3D(event: Render3DEvent) {
        if (circleValue.get() && !circleRainValue.get()) {
            GL11.glPushMatrix()
            GL11.glTranslated(
        ...
        if (target != null && attackTimer.hasTimePassed((attackDelay.toDouble() * 0.9).toLong()) && (autoBlockValue.equals("Range") && canBlock) && autoBlockPacketValue.equals("KeyBlock")) {
            mc.gameSettings.keyBindUseItem.pressed = false
        }

        if (target != null && delayBlockTimer.hasTimePassed(30) && (autoBlockValue.equals("Range") && canBlock)) {
        ...
        discoveredTargets.forEach {
            when (markValue.get().toLowerCase()) {
                "sinka" -> {
                    ...
                    GL11.glTranslated(
                        it.lastTickPosX + (it.posX - it.lastTickPosX) * event.partialTicks - mc.renderManager.viewerPosX,
                        (it.lastTickPosY + (it.posY - it.lastTickPosY) * event.partialTicks - mc.renderManager.viewerPosY) + it.height * 0.4,
                        it.lastTickPosZ + (it.posZ - it.lastTickPosZ) * event.partialTicks - mc.renderManager.viewerPosZ
                    )
                    GL11.glRotatef(-it.width, 0.0f, 1.0f, 0.0f)
                    GL11.glRotatef((mc.thePlayer.ticksExisted + mc.timer.renderPartialTicks) * 5, 0f, 1f, 0f)
                    RenderUtils.glColor(Color(color3))
                    RenderUtils.enableSmoothLine(1.5F)
                    val c = Cylinder()
                    GL11.glRotatef(-90.0f, 1.0f, 0.0f, 0.0f)
                    c.draw(0F, radius1, 0.3f, side, 1)
                    c.drawStyle = 100012
                    GL11.glTranslated(0.0, 0.0, 0.3)
                    c.draw(radius1, 0f, 0.3f, side, 1)
    ...
    override val tag: String
        get() = when (displayMode.get().toLowerCase()) {
