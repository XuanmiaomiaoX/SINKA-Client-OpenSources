...
import net.ccbluex.liquidbounce.utils.Skid.PlaceRotation
...
@ModuleInfo(name = "Scaffold", description = "Scaffold" ,category = ModuleCategory.WORLD, keyBind = Keyboard.KEY_I)
class Scaffold : Module() {
    ...
    private val aurahelper = BoolValue("AuraHelper",true)
    ...
    // Rotations
    private val rotationsValue = ListValue("Rotations", arrayOf("None", "Vanilla", "AAC", "HytAAC5" ,"Test1", "Test2","Legit"), "AAC")
    ...
    private fun move() {
        when (towerModeValue.get().toLowerCase()) {
            ...
            "NCP" -> {
                ...
                val IntPosY = mc.thePlayer.posY.toInt()
                if (mc.thePlayer.posY - IntPosY < 0.05) {
                    mc.thePlayer.setPosition(mc.thePlayer.posX, IntPosY.toDouble(), mc.thePlayer.posZ)
                    mc.thePlayer.motionY = 0.42
                    towerTicks = 1
    ...
    fun onRender3D(event: Render3DEvent?) {
        ...
        for (i in 0 until (expandLengthValue.get() + 1)) {
            ...
            if (BlockUtils.isReplaceable(blockPos) && placeInfo != null) {
                RenderUtils.drawBlockBox2(blockPos, Color(rainbow.red,rainbow.green,rainbow.blue, a.get()), outline.get(), true,1F)
                break
            }
        }
    }
            ...
            when (rotationsValue.get().toLowerCase()) {
                ...
                "hytaac5" ->{
                    if(MovementUtils.isMoving()) {
                        ...
                        RotationUtils.setTargetRotation(Rotation(mc.thePlayer.rotationYaw + DiffYaw, 86F))
                    }
                }
            }
            if (rotation != null) {
                ...
                if (silentRotationValue.get()) {
                    val limitedRotation = RotationUtils.limitAngleChange(RotationUtils.serverRotation, rotation, getSpeed())
                    RotationUtils.setTargetRotation(limitedRotation, keepLengthValue.get())
                } else {
    ...
    fun drawTip() {
        ...
        RenderUtils.drawRoundRect((width - w2 - 20) / 2f, height * 0.8f - 24f, (width + w2 + 18) / 2f, height * 0.8f + 12f, 5f, Color(ClientColor.red, ClientColor.green, ClientColor.blue, 100).rgb
        )
        var stack = barrier
        if (slot != -1) {
        ...
        Fonts.roboto35.drawString(info, width / 2f - 5f, height * 0.8f, Color(ClientColor.red1,ClientColor.green1,ClientColor.blue1).rgb, false)
