...
class AimBot : Module() {
    ...
    private val maxTurnSpeed: IntegerValue = object : IntegerValue("MaxTurnSpeed", 30, 1, 180) {
        override fun onChanged(oldValue: Int, newValue: Int) {
            val minDelayValueObject = minTurnSpeed.get()
            if (minDelayValueObject > newValue) set(minDelayValueObject)
            delay = TimeUtils.randomDelay(minTurnSpeed.get(), this.get())
        }
    }

    private val minTurnSpeed: IntegerValue = object : IntegerValue("MinTurnSpeed", 15, 1, 180) {
        override fun onChanged(oldValue: Int, newValue: Int) {
            val maxDelayValueObject = maxTurnSpeed.get()
            if (maxDelayValueObject < newValue) set(maxDelayValueObject)
            delay = TimeUtils.randomDelay(this.get(), maxTurnSpeed.get())
        }
    }
    ...
    private var delay = TimeUtils.randomDelay(minTurnSpeed.get(), maxTurnSpeed.get())
    ...
    fun onStrafe(event: StrafeEvent) {
        ...
        val entity = mc.theWorld.loadedEntityList.filter {
            EntityUtils.isSelected(it, true) && mc.thePlayer.canEntityBeSeen(it) && mc.thePlayer.getDistanceToEntityBox(it) <= range && RotationUtils.getRotationDifference(it) <= fovValue.get()
                }
