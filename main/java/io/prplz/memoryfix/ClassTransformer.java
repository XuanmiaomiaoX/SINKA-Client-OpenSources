    ...
    private byte[] transformCapeUtils(byte[] bytes) {
        ...
        RemappingClassAdapter adapter = new RemappingClassAdapter(classWriter, new Remapper() {
            public String map(String typeName) {
                return typeName.equals("CapeUtils$1") ? "io.prplz.memoryfix.CapeImageBuffer".replace('.', '/') : typeName;
            }
        });
        ClassReader classReader = new ClassReader(bytes);
        ...
        if ((method.name.equals("getPackImage") || method.name.equals("func_110586_a")) && method.desc.equals("()Ljava/awt/image/BufferedImage;")) {
            ...
            while(iter.hasNext()) {
                ...
                if (insn.getOpcode() == 176) {
                    method.instructions.insertBefore(insn, new MethodInsnNode(184, "io.prplz.memoryfix.ResourcePackImageScaler".replace('.', '/'), "scalePackImage", "(Ljava/awt/image/BufferedImage;)Ljava/awt/image/BufferedImage;", false));
                }
            }
        }
